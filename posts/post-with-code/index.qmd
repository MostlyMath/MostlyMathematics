...
The ergodic theorem was proved in 1931, first by J. von Neumann in the case of ${\mathcal{L}}^2$ convergence and then by D. Birkhoff in the much stronger case of pointwise convergence which solved the most fundamental question in statistical mechanics: when does the time average converges to the space average? Before getting into this let us think about what [Poincaré Recurrence](\posts\my-first-post) gave us:
...
So what we have is a measure preserving system, then we can take a measurable set $\mathcal{E}$ with a point $x \in \mathcal{E}$ which returns infinitely often to $\mathcal{E}$. Poincaré Recurrence guarantees the existence of such a point. What we want to find now is in some sense: the frequency of the occurence of the points of the orbit of $x$ in $\mathcal{E}$. 

Let $\mathcal{O}(x) = \{ x, T(x), T^2(x), \dots \}$ be the orbit of $x \in \mathcal{E}$. Let $\mathcal{O}_N(x) := \{ x, \dots , T^{N-1}(x) \}$

What we want to find is the frequency of the number of points of $\mathcal{O}_N(x)$ that are also in $E$ which can be represented as follows:
$$ \frac{1}{N} \sum_{i=0}^{N-1} X_E(T^i(x)), $$
where $X_E$ is the characteristic function of $E$. Now we could wonder what would happen if $N$ keeps increasing. Consider the following limit:
...
Now we wonder what happens if we do not have an ergodic system. Turns out, in the case of measure-preserving systems, the time average does converge, not to the space average, but to a $T$-invariant function as stated below and we can also very easily find the integral of this limit function.
...
The proof is quite lengthy and I am skipping it here. One can refer to [@silva2008] for a beginner-friendly proof. [@assani2003wiener] has 3 very interesting proofs (one using the maximal inequality, one using a combinatorial argument and the last using a variational inequality) which are a little advanced.
